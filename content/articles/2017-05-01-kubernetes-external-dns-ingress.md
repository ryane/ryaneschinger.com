...
* You must tell ExternalDNS which domain to use. This is done with the `--domain-filter` argument. The Deployment is configured to read this domain from a [ConfigMap](https://kubernetes.io/docs/tasks/configure-pod-container/configure-pod-configmap/) that we will create in the next step.
